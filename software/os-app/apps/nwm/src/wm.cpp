...
Window *WindowManager::spawn(const char *path, const char *argv[]) {
  ...
  char navyhome[128];
  snprintf(navyhome, sizeof(navyhome), "OS_APP_PATH=%s", getenv("OS_APP_PATH"));
#ifdef __ISA_NATIVE__
  char ld_preload[256];
  snprintf(ld_preload, sizeof(ld_preload), "LD_PRELOAD=%s/libs/libos/build/native.so", getenv("OS_APP_PATH"));
#endif
  const char *envp[] = {
    "NWM_APP=1",
#ifdef __ISA_NATIVE__
