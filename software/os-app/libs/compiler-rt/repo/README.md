...
```c
// a * b, overflow set to one if result not in signed range
...
```c
// __clear_cache() is used to tell process that new instructions 
// have been written to an address range.  Necessary on processors 
// that do not have a unified instruction and data cache
